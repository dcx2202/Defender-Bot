...
public class Robo
{
	...
	public void escolheEstrategia()
	{
		...
		TreeMap<Integer, Inimigo>  inimigos_aux = copiaTreemap(Projeto.inimigos);
		...
		else	//Atacar
		{
			for(Inimigo inimigo : inimigos_aux.values())	 //Para cada inimigo
			{
				...
				{
					if(Projeto.turno != 12)	//Se nao for o ultimo turno entao aplica as reservas de energia
					{
						if(inimigo.getVida() > VIDA_INI_GRUA && energAtual > ENERGGRUA + ENERG_ATAQUE_RES)
						{
		...
		if(score <= 0)		//Se o score for negativo (robo morre) entao volta a simular aplicando outro perfil
		{
			...
			inimigos_aux = copiaTreemap(Projeto.inimigos);
			...
			score_aux = calculaScore(inimigos_aux, vidaAtual, energAtual);	//Calcula o score usando o perfil equilibrado
			
			if(score_aux > score)	//Se for melhor que o previamente calculado fica com este
			{
				score = score_aux;
				perfil_escolhido = "equilibrado";	//Indica que o perfil escolhido ate agora e o "equilibrado"
			}
			...
			score_aux = calculaScore(inimigos_aux, vidaAtual, energAtual);	//Calcula o score usando o perfil passivo
			
			if(score_aux > score)	//Se for melhor que o previamente calculado fica com este
			{
				score = score_aux;
				perfil_escolhido = "passivo";	//Indica que o perfil escolhido ate agora e o "passivo"
			}
			...
			score_aux = calculaScore(inimigos_aux, vidaAtual, energAtual);	//Calcula o score usando o perfil sosom
			
			if(score_aux > score)	//Se for melhor que o previamente calculado fica com este
			{
				score = score_aux;
				perfil_escolhido = "sosom";	//Indica que o perfil escolhido ate agora e o "sosom"
			}
		}
